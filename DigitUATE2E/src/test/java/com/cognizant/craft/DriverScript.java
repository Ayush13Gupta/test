...
public class DriverScript {
	...
	case DB:
	    report.addTestLogSubHeading("Execution Mode", ": " + "DataBase Query", "Execution on",
		    ": " + "Local Machine");
	    break;
	case LOCAL:
	...
	if (properties.getProperty("SpecificDataSheet").toUpperCase().contains("YES")) {

	    return setEnvDataSheet(getCurrentSheetName());
	} else {

	    return properties.getProperty("DefaultDataSheet");
	}
    }
	...
	// Error reporting
	String exceptionDescription = Utility_Functions.xExptnsMsg(ex.getMessage());// Updated on 17th May 2020
    ...
    /**
     * Return Data sheet which holds test case test data..
     *
     * @return
     */
	...
	return testDataAccess.getSpecificWorkBookName(testParameters.getCurrentTestcase());
	...
	testDataAccess.setDatasheetName(currentsheet());
	...
	scriptHelper = new ScriptHelper(dataTable, report, driver, driverUtil, apiDriver, extentTest, reusableHandle,
		testParameters.getEndIteration(), currentsheet());
    ...
    public String setEnvDataSheet(String sheetNm) {
	try {
	    String env = System.getProperty("ENV");
	    env = env == null ? properties.getProperty("ENV").toUpperCase() : env.toUpperCase();
	    String shtName = "";
	    if (env.contains("STG")) {
		if (sheetNm.contains("_STG")) {
		    shtName = sheetNm;
		} else if (sheetNm.contains("_QA")) {
		    shtName = sheetNm.replace("_QA", "_STG");
		} else {
		    shtName = sheetNm;
		}

	    } else if (env.contains("QA")) {
		if (sheetNm.contains("_QA")) {
		    shtName = sheetNm;
		} else if (sheetNm.contains("_STG")) {
		    shtName = sheetNm.replace("_STG", "_QA");
		} else {
		    shtName = sheetNm;
		}
	    }
