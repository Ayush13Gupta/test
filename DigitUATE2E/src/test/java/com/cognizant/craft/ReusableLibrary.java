...
public abstract class ReusableLibrary {
	...
	public ReusableLibrary(ScriptHelper scriptHelper) {
		...
		this.endIteration = scriptHelper.getEnditeration();
		...
		this.currentDataSheet = scriptHelper.getCurrentSheet();
	...
	/**
	 *
	 * Method to get data from Data sheet and Json file.
	 *
	 * @param workbook Pass workbook name
	 * @param key      Pass column value , should also existing in Json file
	 * @return
	 */
	protected String getTblDataNdJson(String workbook, String key) {

		if (endIteration > 1) {
			return dataTable.getData(workbook, key);
		} else {
			return Utility_Functions.xGetJsonData(key);
		}
	...
	protected boolean xWaitForElementPresent(By locator, int timeWait) {
		try {
			FluentWait<WebDriver> wait = new WebDriverWait(driver.getWebDriver(), timeWait)
